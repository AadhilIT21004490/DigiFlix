...
const categories = ["All", "Flyers", "Web Development", "Branding", "Package Design"]
const portfolioData = [
  {
    id: 1,
    category: "Flyers",
    image: "/assets/img/case/study-grid1.png",
    label: "Creative",
...
const filteredData = activeCategory === "All" ? portfolioData : portfolioData.filter(item => item.category === activeCategory)
    ...
    <Layout headerStyle={2} footerStyle={3} breadcrumbTitle="Portfolio">
      <section className="case-study pt-10 pb-space">
        <div className="container">
            ...
            <p>We’ve successfully delivered a wide range of diverse projects each one a reflection of our 
            versatility, creativity, and unwavering commitment to excellence. From branding and web 
            development to full-scale digital campaigns, our portfolio showcases the impact we create 
          ...
          <div className="row g-xxl-7 g-xl-6 g-6">
            {filteredData.map((item, idx) => (
              <div key={item.id} className="col-lg-6 col-md-6" data-aos="zoom-in-left" data-aos-duration={1400 + idx * 200}>
                <div className="blog-widget-item bb-border pb-xxl-7 pb-xl-6 pb-4">
                  <div className="thumb w-100 overflow-hidden mb-xxl-7 mb-xl-6 mb-4">
                    <img src={item.image} alt="img" className="w-100 overflow-hidden" />
                  </div>
                  <div className="blog-cont">
                    <div className="d-flex align-items-center gap-xxl-5 gap-xl-4 gap-3 mb-xxl-5 mb-xl-4 mb-3">
                      <Link href={item.link} className="radius-btn cmn-border radius100 py-xxl-2 py-2 px-xxl-4 px-3 theme-clr style-2">
                        {item.label}
                    ...
                    <h5>
                      <Link href={item.link} className="white-clr d-flex align-items-center justify-content-between gap-3">
                        {item.title}
